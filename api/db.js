...
// LEGEND (PT): Camada de proxy para o Supabase (banco de dados online).
//   - Traduz queries SQL (formato SQLite) para chamadas da API do Supabase
//   - Exporta objetos proxy para as tabelas: users, transactions, goals
//   - Cada proxy oferece: query(), getAsync(), allAsync()
//   - Permite trocar o backend sem alterar server.js ou api/index.js
// ============================================================
...
// Fábrica para criar um proxy que espelha a API antiga do SQLite (query, getAsync, allAsync)
// Isso permite que o restante do código continue usando SQL enquanto os dados estão no Supabase.
function createSupabaseProxy(table) {
    return {
